...
test("server-renders the event directory", async () => {
  ...
  assert.doesNotMatch(html, /See what changed and what conflicts\./);
  ...
  assert.match(html, /Open your event, check the agenda, and read the TL;DR/);
  ...
  assert.match(html, /Find the event plan/);
...
test("server-renders the source monitor and approval queue", async () => {
  ...
  assert.match(html, /<span>Needs review<\/span><strong>0<\/strong>/);
  assert.match(html, /<span>No change<\/span><strong>0<\/strong>/);
  ...
  assert.match(html, /Not due · no due or overdue events · Aug 8/);
  assert.match(html, /Private self-DM checked · no event decision · Aug 8/);
  ...
  assert.match(html, /Latest evidence refresh · Scheduled heartbeat: Aug 08, 2026 · 9:12 AM PT · scheduled source scan/);
  ...
  assert.doesNotMatch(html, /Customer Connect priorities · organizer call/);
...
test("server-renders the leadership portfolio without unsupported ROI claims", async () => {
  ...
  assert.equal(response.status, 200);
  const html = await response.text();
  assert.match(html, /The program, the next moves, and the results\./);
  assert.match(html, /id="leadership-snapshot"/);
  ...
  assert.match(html, /Completed closeouts/);
  assert.doesNotMatch(html, /Source checks due|Upstream queue|Latest source work|What to trust|Applied updates|Needs review/);
...
test("server-renders searchable event outcomes and filter counts", async () => {
  ...
  assert.match(latestScanSearchHtml, /0 findings · 0 need review · 0 no change/);
...
test("server-renders a searchable marketing support board", async () => {
  ...
  assert.match(html, /All (?:<!-- -->)?22(?:<!-- -->)? source-based opportunities lack a reportable amount/);
  assert.match(html, /Closed Lost and Disqualified records are excluded/);
...
test("server-renders field-role CRM rules and the updated guide model", async () => {
  ...
  assert.match(guidesHtml, /Use this page for the steps that apply to every event/);
  ...
  assert.match(guidesHtml, /Open your event page for its agenda, team, and specific plan/);
