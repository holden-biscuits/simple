...
test("server-renders the source monitor and approval queue", async () => {
  ...
  assert.match(html, /<span>Applied<\/span><strong>8<\/strong>/);
...
test("server-renders the leadership portfolio without unsupported ROI claims", async () => {
  ...
  assert.match(html, /8<\/strong><span>Applied updates/);
...
test("server-renders a searchable marketing support board", async () => {
  ...
  assert.match(html, /13(?:<!-- -->)? active events need canonical HubSpot event coverage/);
...
test("server-renders dynamic event facts without empty filler notes", async () => {
  ...
  assert.match(chicagoHtml, /Taylor is the confirmed attendee in the tracker/);
  assert.match(chicagoHtml, /Carter and Josh remain marked available/);
