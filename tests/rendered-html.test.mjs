...
test("server-renders dynamic event facts without empty filler notes", async () => {
  ...
  assert.match(chicagoHtml, /Onsite check/);
  assert.match(chicagoHtml, /Onsite gaps open/);
  assert.match(chicagoHtml, /A source conflict still affects this brief/);
  assert.match(chicagoHtml, /1 attendee name is still missing from the onsite roster/);
  assert.match(chicagoHtml, /Venue details are not recorded/);
  assert.match(chicagoHtml, /Pass and credential details are not recorded/);
  assert.match(chicagoHtml, /6(?:<!-- -->)? onsite facts unresolved/);
  assert.doesNotMatch(chicagoHtml, /Turn the open priorities into owned, dated tasks/);
  assert.match(chicagoHtml, /Due[\s\S]*Today/);
  assert.match(chicagoHtml, /Status[\s\S]*Owner open/);
  assert.match(chicagoHtml, /href="#event-priorities">Open onsite priorities(?:<!-- -->)? →/);
  assert.match(chicagoHtml, /Onsite focus/);
  assert.match(chicagoHtml, /Do these next\./);
  assert.match(chicagoHtml, /4(?:<!-- -->)? event-specific (?:<!-- -->)?priorities are live today(?:<!-- -->)?\./);
  assert.match(chicagoHtml, /href="#workstream-speaking"[^>]*>Open field checklist(?:<!-- -->)? →/);
  assert.doesNotMatch(chicagoHtml, /Turn priorities into a plan/);
