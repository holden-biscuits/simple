...
test("anchored navigation clears sticky chrome at each breakpoint", () => {
  ...
  assert.match(css, /\.action-briefing\[id\]/);
...
test("the program pulse keeps its hierarchy on tablet and mobile", () => {
  ...
  assert.match(css, /\.attention-filters > div\s*\{[^}]*grid-template-columns:\s*repeat\(6,/);
  ...
  assert.match(css, /@media \(max-width: 620px\)[\s\S]*\.attention-filters button:last-child\s*\{\s*grid-column:\s*auto;/);
