...
test("directory state is bounded and serializes into a shareable event view", () => {
  const state = parseEventDirectoryState({ q: " Genesys ", attendance: "going", year: "2027" }, years);
  assert.deepEqual(state, { query: "Genesys", attendance: "going", year: "2027" });
  assert.equal(getEventDirectoryHref(state), "/?q=Genesys&attendance=going&year=2027#events");
  assert.equal(getEventDetailHref("genesys-xperience", state), "/events/genesys-xperience?returnTo=%2F%3Fq%3DGenesys%26attendance%3Dgoing%26year%3D2027%23events");
});

test("invalid filters fall back to the full directory", () => {
  const state = parseEventDirectoryState({ attendance: "maybe", year: "2030" }, years);
  assert.deepEqual(state, { query: "", attendance: "all", year: "all" });
