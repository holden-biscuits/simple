...
export default async function EventPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main>
      ...
      <section className="fact-strip shell">{facts.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</section>
      ...
      <section className="shell event-body">
        ...
        <div className="workstreams">
          ...
          {(Object.keys(workstreamLabels) as WorkstreamKey[]).map((key, index) => {
            ...
            return <article className={`workstream ${isNone ? "workstream-none" : ""}`} key={key}>
              <span>{String(index + 1).padStart(2, "0")}</span>
              <div><h3>{workstreamLabels[key]}</h3><ul>{items.map((item) => <li key={item}>{item}</li>)}</ul></div>
            </article>;
          })}
        </div>
      </section>
